 ...
 */
template<typename K, size_t EpsilonRecursive = 64, typename Floating = float>
class PGMIndex {
public:
    ...
    auto segment_for_key(const K &key) const {
        ...
        auto it = segments.begin() + *(levels_offsets.end() - 2); // 最高的一层level的offset起始位置, 最高层只有一个模型，另一个是哨兵
        for (auto l = int(height()) - 2; l >= 0; --l) {
            auto level_begin = segments.begin() + levels_offsets[l];
            auto pos = std::min<size_t>((*it)(key), std::next(it)->intercept); // 预测
            ...
            if constexpr (EpsilonRecursive <= linear_search_threshold) { // 线性查找
...
public:
     ...
     */
    explicit PGMIndex(const std::vector<K> &data, size_t setEpsilon) : PGMIndex(data.begin(), data.end(), setEpsilon) {}
     ...
     */
    template<typename RandomIt>
    PGMIndex(RandomIt first, RandomIt last, size_t setEpsilon)
    ...
    /** rebuild PGM according to new Epsilon **/
    template<typename RandomIt>
    void rebuild(RandomIt first, RandomIt last, size_t new_Epsilon) {
        n = std::distance(first, last);
        first_key = n ? *first : K(0);
     ...
     * @return a struct with the approximate position and bounds of the range
     */
    size_t search_left(const K &key) const {
        auto k = std::max(first_key, key);
        auto it = segment_for_key(k);
        auto pos = std::min<size_t>((*it)(k), std::next(it)->intercept);
        auto lo = PGM_SUB_EPS(pos, Epsilon);
    ...
    void print_info(){
        for(int i = 0; i < levels_offsets.size(); i++){
            std::cout << "level offset " << i << " " << levels_offsets[i] << std::endl;
        }
        auto it = segments.begin() + *(levels_offsets.end() - 2);
        auto it2 = segments.begin() + *(levels_offsets.end() - 2) + 1;
        std::cout << "it k " << it->key << " slope" << it->slope << " intercept " << it->intercept <<  std::endl;
        std::cout << "it2 k " << it2->key << " slope" << it2->slope << " intercept " << it2->intercept <<  std::endl;
    }
};
...
struct PGMIndex<K, EpsilonRecursive, Floating>::Segment {
