...
private:
    ...
    struct Slope {
        ...
        bool operator<(const Slope &p) const { return dy * p.dx < dx * p.dy; } //train_ops+=2;
        bool operator>(const Slope &p) const { return dy * p.dx > dx * p.dy; } //train_ops+=2;
        bool operator==(const Slope &p) const { return dy * p.dx == dx * p.dy; } //train_ops+=2;
        bool operator!=(const Slope &p) const { return dy * p.dx != dx * p.dy; } //train_ops+=2; 
        explicit operator long double() const { return dy / (long double) dx; } //train_ops+=1;
    };
...
size_t make_segmentation_par(size_t n, size_t epsilon, Fin in, Fout out) {
    auto parallelism = std::min(std::min(omp_get_num_procs(), omp_get_max_threads()), 32); // 20 threads
